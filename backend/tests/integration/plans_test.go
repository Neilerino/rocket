...
func (suite *IntegrationTestSuite) TestPlansGetById() {
	...
	recorder := suite.GET("/api/v1/plans/1")
	suite.AssertStatusCode(recorder, 200)
	
	var plan types.Plan
	suite.GetResponseData(recorder, &plan)
	suite.Equal(int64(1), plan.ID, "Plan ID should be 1")
	suite.Equal("User1 Regular Plan", plan.Name, "Plan name should match test data")
	suite.Equal(int64(1), plan.UserID, "Plan should belong to user 1")
	suite.False(plan.IsTemplate, "Plan should not be a template")
	suite.False(plan.IsPublic, "Plan should not be public")

	// Test Case 2: Access to existing plan by different user (plan 5 belongs to user 2)
	recorder = suite.GET("/api/v1/plans/5")
	suite.AssertStatusCode(recorder, 200)
	
	suite.GetResponseData(recorder, &plan)
	suite.Equal(int64(5), plan.ID, "Plan ID should be 5")
	suite.Equal("User2 Plan", plan.Name, "Plan name should match test data")
	suite.Equal(int64(2), plan.UserID, "Plan should belong to user 2")
}
